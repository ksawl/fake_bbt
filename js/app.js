import LightGallery from "./lib/LightGallery.js";
import __webpack_require__ from "./lib/ZoomGallery.js";
...
}
{
  ...
  (() => {
    ...
    (() => {
      ...
      function initBooks() {
        if (document.querySelector("[data-book]")) {
          let id = document.querySelector("[data-book]").dataset.book;
          const pageInfo = bookInfo[id];
          const titles = document.querySelector("[data-tabs-titles]");
          const body = document.querySelector("[data-tabs-body]");
          let version =
            window.location.hash.substring(1) ||
            localStorage.getItem("defaultLanguage");
          for (let value in pageInfo) {
            titles.innerHTML += `<button type="button" class="book__navigation-title"><span>${value}</span></button>`;
            body.innerHTML += `<div data-open-parent class="book__wrapper-body" id="${value.slice(
              1
            )}"></div>`;
          }
          if (imagesInfo[id][0]["show-page"] == "true")
            document
              .querySelector(".book__navigation-title")
              .classList.add("_tab-active");
          let tabsPanels = document.querySelectorAll(".book__wrapper-body");
          let p = 1;
          let i = 0,
            k = 0;
          while (i < tabsPanels.length)
            for (let value in pageInfo) {
              let f = 1;
              for (let key in pageInfo[value]) {
                tabsPanels[i].insertAdjacentHTML(
                  "beforeend",
                  `     \n
                                <section class="book__block book-info ${
                                  k % 2 !== 0 ? "book-info--white" : ""
                                }">\n
                                    <div class="book-info__container">\n
                                        <div class="book-info__image" data-book-gallery>\n
                                            <div class="book-info__main"><img src="./img/book/${
                                              pageInfo[value][key]["images"][0]
                                            }" width="367" height="534" alt="" class="book-info__img"></div>\n
                                            <div class="book-info__mini"><img src="./img/book/${
                                              pageInfo[value][key]["images"][1]
                                                ? pageInfo[value][key][
                                                    "images"
                                                  ][1]
                                                : ""
                                            ...
                                            ${
                                              pageInfo[value][key][
                                                ...
                                                ? '<div class="book-info__tooltip"><span class="book-info__tooltip-title" data-language="coverNeeded-title">Cover required!</span><span class="book-info__tooltip-content" data-language="coverNeeded-content">If you have a better cover for this book, please contact us via the feedback form at <a href="annoncment.html">http://bhaktivedanta-book-trust.com/annoncment.html</a></span></div>'
                                                : ""
                                            }\n
                                        </div>\n
                                        <div class="book-info__content">\n
                                            <div class="book-info__header">\n
                                                <h2 class="book-info__titleimg">\n
                                                    <img data-language="${key}-bookNameImage" src="./img/titles/${
                    pageInfo[value][key]["block-name-image"]
                      ? pageInfo[value][key]["block-name-image"]
                      : "Значение отсутствует"
                  }" alt="">\n
                                                </h2>\n
                                                <h3 class="book-info__title" data-language="${key}-bookTitle">${
                    pageInfo[value][key]["block-title"]
                      ? pageInfo[value][key]["block-title"]
                      : "Значение отсутствует"
                  }</h3>\n
                                                <span class="book-info__subtitle" data-language="${key}-bookSubTitle">${
                    pageInfo[value][key]["block-subtitle"]
                      ? pageInfo[value][key]["block-subtitle"]
                      : "Значение отсутствует"
                  }</span>\n
                                                <span class="book-info__descr" data-language="${key}-bookDescr">${
                    pageInfo[value][key]["block-addition-description"]
                      ? pageInfo[value][key]["block-addition-description"]
                      : "Значение отсутствует"
                  }</span>\n
                                            </div>\n
                                            <table class="book-info__table">\n
                                                <tbody>\n
                                                    <tr>\n
                                                        <td data-language="bookLine-1">Edition</td>\n
                                                        <td data-language="${key}-lineEdition">${
                    pageInfo[value][key]["info-table"][0]
                      ? pageInfo[value][key]["info-table"][0]
                      : "Значение отсутствует"
                  }</td>\n
                                                    </tr>\n
                                                    <tr>\n
                                                        <td data-language="bookLine-2">Pub.Date</td>\n
                                                        <td data-language="${key}-linePubDate">${
                    pageInfo[value][key]["info-table"][1]
                      ? pageInfo[value][key]["info-table"][1]
                      : "Значение отсутствует"
                  }</td>\n
                                                    </tr>\n
                                                    <tr>\n
                                                        <td data-language="bookLine-3">Language</td>\n
                                                        <td data-language="${key}-lineLanguage">${
                    pageInfo[value][key]["info-table"][2]
                      ? pageInfo[value][key]["info-table"][2]
                      : "Значение отсутствует"
                  }</td>\n
                                                    </tr>\n
                                                    <tr>\n
							<td data-language="${key}-infoTableNprinting">${
                    pageInfo[value][key]["info_table_print"]
                      ? pageInfo[value][key]["info_table_print"]
                      : "Значение отсутствует"
                  }</td>\n
                                                        <td data-language="${key}-linePrinting">${
                    pageInfo[value][key]["info-table"][3]
                      ? pageInfo[value][key]["info-table"][3]
                      : "Значение отсутствует"
                  }</td>\n
                                                    </tr>\n
                                                    <tr>\n
                                                        <td data-language="bookLine-5">Printed by</td>\n
                                                        <td data-language="${key}-linePrintedBy">${
                    pageInfo[value][key]["info-table"][4]
                      ? pageInfo[value][key]["info-table"][4]
                      : "Значение отсутствует"
                  }</td>\n
                                                    </tr>\n
                                                    <tr>\n
                                                        <td data-language="bookLine-6">Pages</td>\n
                                                        <td data-language="${key}-linePages">${
                    pageInfo[value][key]["info-table"][5]
                      ? pageInfo[value][key]["info-table"][5]
                      : "Значение отсутствует"
                  }</td>\n
                                                    </tr>\n
                                                    <tr>\n
                                                        <td data-language="bookLine-7">size</td>\n
                                                        <td data-language="${key}-lineSize">${
                    pageInfo[value][key]["info-table"][6]
                      ? pageInfo[value][key]["info-table"][6]
                      : "Значение отсутствует"
                  }</td>\n
                                                    </tr>\n
                                                    <tr>\n
                                                        <td data-language="bookLine-8">lccn</td>\n
                                                        <td data-language="${key}-lineLccn">${
                    pageInfo[value][key]["info-table"][7]
                      ? pageInfo[value][key]["info-table"][7]
                      : "Значение отсутствует"
                  }</td>\n
                                                    </tr>\n
                                                    <tr>\n
                                                        <td data-language="bookLine-9">Reg. num.</td>\n
                                                        <td data-language="${key}-lineRegNum">${
                    pageInfo[value][key]["info-table"][8]
                      ? pageInfo[value][key]["info-table"][8]
                      : "Значение отсутствует"
                  }</td>\n
                                                    </tr>\n
                                                    <tr>\n
                                                        <td data-language="bookLine-10">Translated into</td>\n
                                                        <td data-language="${key}-lineTranslatedInto">${
                    pageInfo[value][key]["info-table"][9]
                      ? pageInfo[value][key]["info-table"][9]
                      : "Значение отсутствует"
                  }</td>\n
                                                    </tr>\n
                                    ...
                                    <button data-open-button class="book-info__more">\n
                                        <span data-language="bookDetails">${
                                          "ru" == version
                                            ? "подробнее"
                                            : "see details"
                                        }</span>\n
                                ...
                                <section class="book-details ${
                                  k % 2 !== 0 ? "book-details--white" : ""
                                }" data-open-body>\n
                                    <div class="book-details__container">\n
                                        <h2 data-language="bookAboutEditionTitle" class="book-details__title">About this edition</h2>\n
                                        <div class="book-details__text">\n
                                            <div class="book-details__table-block">\n
                                                <div class="book-details__slider swiper">\n
                                                    <div class="book-details__wrapper swiper-wrapper">\n
                                                    </div>\n
                                                    <button class="book-details__button book-details__button--prev" data-prev><span>0</span></button>\n
                                                    <button class="book-details__button book-details__button--next" data-next><span></span></button>\n
                                                </div>\n
                                                <div class="book-details__language">\n
                                                    <ul class="book-details__language-list">\n
                                                        <li class="book-details__language-item"><button data-slider-language="French" class="book-details__language-button">FRA</button></li>\n
                                                        <li class="book-details__language-item"><button data-slider-language="Deutsch" class="book-details__language-button">DEU</button></li>\n
                                                        <li class="book-details__language-item"><button data-slider-language="English" class="book-details__language-button chosen">ENG</button></li>\n
                                                        <li class="book-details__language-item"><button data-slider-language="Italian" class="book-details__language-button">ITA</button></li>\n
                                                        <li class="book-details__language-item"><button data-slider-language="Spanish" class="book-details__language-button">ESP</button></li>\n
                                                        <li class="book-details__language-item"><button data-slider-language="Chinese" class="book-details__language-button">CHN</button></li>\n
                                                        <li class="book-details__language-item"><button data-slider-language="Indian" class="book-details__language-button">IND</button></li>\n
                                                        <li class="book-details__language-item"><button data-slider-language="Russian" class="book-details__language-button">RUS</button></li>\n
                                                    </ul>\n
                                                </div>\n
                                            </div>\n
                                            <h2 data-language="bookAboutEditionTitle" class="book-details__title book-details__title--mobile">About this edition</h2>\n
                                            <div data-language="${key}-editionText">${
                    pageInfo[value][key]["block-text"]
                      ? pageInfo[value][key]["block-text"]
                      : "Значение отсутствует"
                  }</div>\n
                                            <!--BUG div data-language="${id}-detailsText-${p}">${
                    pageInfo[value][key]["block-text"]
                      ? pageInfo[value][key]["block-text"]
                      : "Значение отсутствует"
                  }</div-->\n
                                    ...
                                    <button data-close-button class="book-info__more book-info__more--open">\n
                                        <span data-language="bookCollapse">collapse see details</span>\n
                                    </button>\n
                                </section>\n
                        `
                );
      ...
      function initImages(id) {
        let certainPage = imagesInfo[id];
        const imageTitles = document.querySelector("[data-tabs-images]");
        let p = 0;
        for (let i = 0; i < certainPage.length; i++)
          if ("" == certainPage[i]["previewImage"])
            imageTitles.innerHTML += `<div><img src="./img/assets/${
              certainPage[i]["previewImage"]
                ? certainPage[i]["previewImage"]
                : ""
            }" alt="" style="opacity:0;visibility:hidden;" aria--hidden="true"></div>`;
          else {
            imageTitles.innerHTML += `\n         <div class="book__image">\n            <img class="book__image-tooltip" data-popcorn src="./img/assets/${
              certainPage[i]["previewImage"]
                ? certainPage[i]["previewImage"]
                : ""
            }" alt="">\n            <div data-tooltip class="books__tooltip" style="--count: ${
              certainPage[i]["column-count"]
                ? certainPage[i]["column-count"]
                : ""
            }">                \n            </div>\n         </div>`;
            initTooltipImages(
              document.querySelectorAll("[data-tooltip]")[p],
              certainPage[i]
            );
            p++;
          }
      ...
      function initTooltipImages(elem, object) {
        for (let value of object["tooltipImages"])
          elem.insertAdjacentHTML(
            "beforeend",
            `\n         <div class="books__tooltip-image"><img class="books__tooltip-img" src="./img/assets/${value}" alt=""></div>\n      `
          );
      }
      function initTooltips() {
        const popcorns = document.querySelectorAll("[data-popcorn]");
        popcorns.forEach((el) => {
          const tooltip = el.offsetParent.querySelector("[data-tooltip]");
          popper_createPopper(el, tooltip, {
      ...
      function scrollToBlock() {
        if (localStorage.getItem("scrollTo")) {
          let object = JSON.parse(localStorage.getItem("scrollTo"));
          const tabsTitles =
            document.querySelector("[data-tabs-titles]")?.children;
          document.querySelectorAll(".book__navigation-title");
          const tabsBodies = document.querySelectorAll(".book__wrapper-body");
          if (tabsTitles[object.tab]) tabsTitles[object.tab].click();
          if (tabsBodies[object.tab]) {
            const sections =
              tabsBodies[object.tab].querySelectorAll(".book-info");
            setTimeout(() => {
              window.scrollTo({
                top: sections[object.section].offsetTop,
                left: sections[object.section].offsetLeft,
                behavior: "smooth",
              });
            }, 500);
          }
      ...
      function initImageConnection() {
        const tabsImages = document.querySelectorAll(".book__image");
        const tabsTitles =
          document.querySelector("[data-tabs-titles]")?.children;
        const imagesTooltips = document.querySelectorAll(".books__tooltip");
        const tabsBlocks = document.querySelectorAll(".book__wrapper-body");
        if (tabsImages.length && tabsImages.length === tabsTitles.length)
          for (let i = 0; i < tabsImages.length; i++)
            tabsImages[i].addEventListener("click", () => {
              tabsTitles[i].click();
            });
        if (imagesTooltips.length === tabsBlocks.length)
          imagesTooltips.forEach((el, i) => {
            const sections = tabsBlocks[i].querySelectorAll(".book-info");
            const buttons = el.querySelectorAll(".books__tooltip-image");
            if (buttons.length && sections.length)
              buttons.forEach((el, j) => {
                el.addEventListener("click", () => {
                  if (tabsBlocks[i].hidden) {
                    el.closest(".book__image").click();
                    setTimeout(() => {
                      window.scrollTo({
                        top: sections[j].offsetTop,
                        left: sections[j].offsetLeft,
                        behavior: "smooth",
                      });
                    }, 500);
                  } else
      ...
      function initTables(id) {
        const sliders = document.querySelectorAll(".book-details__wrapper");
        let certainPage = tablesInfo[id];
        let p = 1;
        let i = 0;
        while (i < sliders.length)
          for (let value in certainPage) {
            let f = 1;
            if (certainPage[`table-${i + 1}`])
              for (let key in certainPage[value]) {
                sliders[i].insertAdjacentHTML(
                  "beforeend",
                  `\n
			<div class="book-details__slide swiper-slide swiper-slide-active" data-book-language="${
        certainPage[value][key]["translation-language"]
          ? certainPage[value][key]["translation-language"]
          : "english"
      }" data-popup-number=${id}-popup-${p} data-popup="#book">\n
			    <div class="book-details__line">\n
				<div>\n
				    <h3 data-language="${key}-tableNprinting">${
                    certainPage[value][key]["printings_name"]
                      ? certainPage[value][key]["printings_name"]
                      : "Значение отсутствует"
                  }</h3>\n
				    <p data-language="${key}-tableTypography">${
                    certainPage[value][key]["typography"]
                      ? certainPage[value][key]["typography"]
                      : "Значение отсутствует"
                  }</p>\n
				</div>\n
				<div>\n
				    <span style="font-size: 24px" data-language="${key}-tableYearPrint">${
                    certainPage[value][key]["year_of_printing"]
                      ? certainPage[value][key]["year_of_printing"]
                      : "Значение отсутствует"
                  }</span>\n
				    <!--span data-language="${key}-tableYearPrint">${
                    certainPage[value][key]["year_of_printing"]
                      ? certainPage[value][key]["year_of_printing"]
                      : "Значение отсутствует"
                  }</span-->\n
				...
				<div>\n
				    <h3 data-language="${key}-tableCopies">${
                    certainPage[value][key]["number_of_copies"]
                      ? certainPage[value][key]["number_of_copies"]
                      : "Значение отсутствует"
                  }</h3>\n
				    <p data-language="${key}-tableBookProperties">${
                    certainPage[value][key]["book_properties"]
                      ? certainPage[value][key]["book_properties"]
                      : "Значение отсутствует"
                  }</p>\n
				</div>\n
                ...
                initPopup(
                  document.querySelector(
                    `.book-details__slide[data-popup-number=${id}-popup-${p}]`
                  ),
                  certainPage[value][key]
                );
                p++;
                f++;
              }
        ...
        initMoreAnimation();
        scrollToBlock();
        languageFilter(certainPage);
        initGalleries();
      }
      function initMoreAnimation() {
        const openParents = document.querySelectorAll("[data-open-parent]");
        if (openParents.length) {
          openParents.forEach((parent) => {
            const buttons = parent.querySelectorAll("[data-open-button]");
            const closeButtons = parent.querySelectorAll("[data-close-button]");
            const bodies = parent.querySelectorAll("[data-open-body]");
            buttons.forEach((button, i) => {
              const description = button.querySelector("span");
              const body = bodies[i];
              const nextButton = body.querySelector("[data-next]");
              const slidesLength = body.querySelectorAll(
                ".book-details__slide"
              ).length;
          ...
          function toggleTwoClasses(
            ...
            if (!element.classList.contains(first)) {
              ...
              description.textContent =
                "ru" == window.location.hash.substring(1) ||
                "ru" == localStorage.getItem("defaultLanguage")
                  ? "закрыть"
                  : "collapse see details";
              button.classList.add("book-info__more--open");
              element.style.maxHeight =
                element.querySelector(".book-details__container").scrollHeight +
                "px";
              window.addEventListener("resize", () => {
                element.style.maxHeight =
                  element.querySelector(".book-details__container")
                    .scrollHeight + "px";
              });
            } else {
              description.textContent =
                ...
                "ru" == localStorage.getItem("defaultLanguage")
                  ? "подробнее"
                  : "see details";
              button.classList.remove("book-info__more--open");
      ...
      function languageFilter(object) {
        const languageLists = document.querySelectorAll(
        ...
        if (languageLists.length)
          languageLists.forEach((list) => {
            const languageButtons = list.querySelectorAll(
              "[data-slider-language]"
            );
            const sliderLines = list
              .closest(".book-details__table-block")
              .querySelectorAll("[data-book-language]");
            const sliderWrapper = list
              .closest(".book-details__table-block")
              .querySelector(".book-details__wrapper");
            const prevButton = list
              .closest(".book-details__table-block")
              .querySelector(".book-details__button--prev");
            const nextButton = list
              .closest(".book-details__table-block")
              .querySelector(".book-details__button--next");
            localStorage.setItem("translate", "english");
            let count = {};
            let arr = [];
            for (let i = 0; i < sliderLines.length; i++) {
              arr.push(sliderLines[i].dataset.bookLanguage);
              if (!count[`${sliderLines[i].dataset.bookLanguage}`])
                count[
                  `${sliderLines[i].dataset.bookLanguage.toLowerCase()}`
                ] = 1;
              else
                count[
                  `${sliderLines[i].dataset.bookLanguage.toLowerCase()}`
            ...
            nextButton.addEventListener("click", () => {
              let text = nextButton.querySelector("span");
              let textContent = text.textContent;
              if (textContent - 1 > -1) text.textContent--;
            });
            ...
            languageButtons.forEach((button) => {
              if (!arr.includes(button.dataset.sliderLanguage.toLowerCase()))
                button.parentElement.style.display = "none";
              if ("english" == button.dataset.sliderLanguage.toLowerCase()) {
                button.parentElement.classList.add("chosen");
                button.textContent = languageChange["english"][0];
              }
              if (count["english"] - 4 > 0) {
                nextButton.querySelector("span").textContent =
                  count["english"] - 4;
                nextButton.disabled = false;
                nextButton.classList.remove("swiper-button-disabled");
              } else {
                nextButton.querySelector("span").textContent = "0";
                nextButton.disabled = true;
                nextButton.classList.add("swiper-button-disabled");
              }
              sliderLines.forEach((el) => {
                if ("english" !== el.dataset.bookLanguage.toLowerCase())
                  el.style.display = "none";
              });
              button.addEventListener("click", () => {
                localStorage.setItem(
                  "translate",
                  button.dataset.sliderLanguage
                );
                prevButton.children[0].textContent = "0";
                sliderWrapper.style.transform = "translate3d(0px, 0px, 0px)";
                languageButtons.forEach((el) => {
                  el.parentElement.classList.remove("chosen");
                  el.textContent =
                    languageChange[el.dataset.sliderLanguage.toLowerCase()][1];
                });
                button.parentElement.classList.add("chosen");
                button.textContent =
                  languageChange[
                    button.dataset.sliderLanguage.toLowerCase()
                  ][0];
                if (
                  count[button.dataset.sliderLanguage.toLowerCase()] - 4 >
                  0
                ) {
                  nextButton.querySelector("span").textContent =
                    count[button.dataset.sliderLanguage.toLowerCase()] - 4;
                ...
                sliderLines.forEach((el) => {
                  ...
                  if (
                    el.dataset.bookLanguage.toLowerCase() !==
                    button.dataset.sliderLanguage.toLowerCase()
                  )
                    el.style.display = "none";
                });
      ...
      function initGalleries() {
        const galleries = document.querySelectorAll("[data-book-gallery]");
        galleries.forEach((gallery) => {
          const mainImage = gallery.querySelector(".book-info__main img");
          const miniImages = gallery.querySelectorAll(".book-info__mini img");
          miniImages.forEach((image) => {
            image.addEventListener("click", () => {
              miniImages.forEach((el) => {
                el.parentElement.classList.remove("clicked");
              });
              mainImage.src = image.src;
              image.parentElement.classList.add("clicked");
            });
          });
        });
      }
      function initPopup(elem, object) {
        const popup = document.querySelector(".popup--book#book");
        const popupImageTitle = popup.querySelector(".book-info__titleimg img");
        const popupTitle = popup.querySelector(".book-info__title");
        const popupSubTitle = popup.querySelector(".book-info__subtitle");
        const popupDescr = popup.querySelector(".book-info__descr");
        const popupText = popup.querySelector(".book-details__text");
        const popupPrinting = popup.querySelector("#printing");
        const popupImages = popup.querySelectorAll(".book-info__image img");
        const popupTableLast = popup.querySelectorAll(
          ".book-info__table tr td:last-child"
        );
        elem.addEventListener("click", () => {
          let version =
            window.location.hash.substring(1) ||
            localStorage.getItem("defaultLanguage");
          popupTitle.innerHTML = object[`block-title-${version}`]
            ? object[`block-title-${version}`]
            : object["block-title"]
            ? object["block-title"]
            : "Значение отсутствует";
          popupImageTitle.src = `./img/titles/${
            object[`block-name-image-${version}`]
              ? object[`block-name-image-${version}`]
              : object["block-name-image"]
              ? object["block-name-image"]
              : "./img/titles/01.svg"
          }`;
          popupSubTitle.innerHTML = object[`block-subtitle-${version}`]
            ? object[`block-subtitle-${version}`]
            : object["block-subtitle"]
            ? object["block-subtitle"]
            : "Значение отсутствует";
          popupDescr.innerHTML = object[`block-addition-description-${version}`]
            ? object[`block-addition-description-${version}`]
            : object["block-addition-description"]
            ? object["block-addition-description"]
            : "Значение отсутствует";
          popupText.innerHTML = object[`block-text-${version}`]
            ? object[`block-text-${version}`]
            : object["block-text"]
            ? object["block-text"]
            : "Значение отсутствует";
          popupPrinting.innerHTML = object[`info_table_print-${version}`]
            ? object[`info_table_print-${version}`]
            : object["info_table_print"]
            ? object["info_table_print"]
            : "Значение отсутствует";
          if (object["cover-needed"] == "true") {
            const image = popup.querySelector(".book-info__image");
            //                    image.insertAdjacentHTML("beforeend", `<div class="book-info__tooltip"><span class="book-info__tooltip-title" data-language="coverNeeded-title">Cover Required!</span><span class="book-info__tooltip-content" data-language="coverNeeded-content">If you have a better cover for this book, please contact us via the feedback form at <a href="annoncment.html">http://bhaktivedanta-book-trust.com/annoncment.html</a></span></div>`);
            image.insertAdjacentHTML(
              "beforeend",
              `<div class="book-info__tooltip"><span class="book-info__tooltip-title" data-language="coverNeeded-title">${
                languages["coverNeeded-title"][
                  "ru" == window.location.hash.substring(1) ||
                  "ru" == localStorage.getItem("defaultLanguage")
                    ? "ru"
                    : "en"
                ]
              }</span><span class="book-info__tooltip-content" data-language="coverNeeded-content">${
                languages["coverNeeded-content"][
                  "ru" == window.location.hash.substring(1) ||
                  "ru" == localStorage.getItem("defaultLanguage")
                    ? "ru"
                    : "en"
          ...
          //                for (let i = 0; i < popupTableLast.length; i++) popupTableLast[i].textContent = object[`info-table-${version}`] ? object[`info-table-${version}`][i] : object["info-table"][i] ? object["info-table"][i] : "Значение отсутствует";
          for (let i = 0; i < popupTableLast.length; i++)
            popupTableLast[i].innerHTML = object[`info-table-${version}`]
              ? object[`info-table-${version}`][i]
              : object["info-table"][i]
              ? object["info-table"][i]
              : "Значение отсутствует";
          for (let i = 0; i < popupImages.length; i++)
            popupImages[i].src = `./img/book/${
              object["images"][i] ? object["images"][i] : ""
            }`;
        });
      }
      function tooltips() {
        const tooltips = document.querySelectorAll(".book-info__tooltip");
        tooltips.forEach((el) => {
          el.addEventListener("click", () => {
      ...
      class Popup {
        ...
        open(selectorValue) {
          if (bodyLockStatus) {
            ...
            if (this.targetOpen.element) {
              ...
              //                        this.popupLogging(this.targetOpen.selector);
              GoToPage(lastpath + ">open" + this.targetOpen.selector);
            } else
              this.popupLogging(
        ...
        close(selectorValue) {
          ...
          GoToPage(lastpath + "<close" + this.targetOpen.selector);
        }
        _getHash() {
          if (this.options.hashSettings.location)
      ...
      function Zoom(_ref) {
        ...
        function zoomToggle(e) {
          const zoom = swiper.zoom;
          if (zoom.scale && 1 !== zoom.scale) zoomOut();
          else zoomIn(e);
        }
      ...
      function initSliders() {
        if (document.querySelector(".trustees__slider-block"))
          new core(".trustees__slider", {
            ...
            lazy: true,
            navigation: {
              prevEl: ".trustees__arrow-left",
              nextEl: ".trustees__arrow-right",
            },
            on: {},
          });
        if (document.querySelector(".beneficiary"))
          new core(".beneficiary__slider", {
            modules: [Navigation, Lazy],
            observer: true,
            observeParents: true,
            slidesPerView: 1,
            ...
            lazy: true,
            navigation: {
              prevEl: ".beneficiary__arrow-left",
              nextEl: ".beneficiary__arrow-right",
            },
            on: {},
          });
        if (document.querySelector(".documents"))
          new core(".documents__slider_1", {
            modules: [Navigation, Lazy],
            observer: true,
            observeParents: true,
            slidesPerView: 1,
            ...
            lazy: true,
            navigation: {
              prevEl: ".documents__arrow-left_1",
              nextEl: ".documents__arrow-right_1",
            },
            on: {},
          });
        if (document.querySelector(".documents"))
          new core(".documents__slider_2", {
            modules: [Navigation, Lazy],
            observer: true,
            observeParents: true,
            slidesPerView: 1,
            ...
            lazy: true,
            navigation: {
              prevEl: ".documents__arrow-left_2",
              nextEl: ".documents__arrow-right_2",
            },
            on: {},
          });
        if (document.querySelector(".documents"))
          new core(".documents__slider_3", {
            modules: [Navigation, Zoom, Lazy],
            observer: true,
            observeParents: true,
            slidesPerView: 1,
            spaceBetween: 10,
            ...
            lazy: true,
            navigation: {
              prevEl: ".documents__arrow-left_3",
              nextEl: ".documents__arrow-right_3",
            },
            on: {},
          });
        if (document.querySelector(".book"))
          new core(".book-details__slider", {
            ...
            direction: "vertical",
            navigation: {
              prevEl: ".book-details__button--prev",
              nextEl: ".book-details__button--next",
            },
            on: {
              init: function (e) {
                this.navigation.$prevEl[0].children[0].textContent =
                  e.activeIndex;
              },
            ...
            if (!stickyConfig.media || stickyConfig.media < window.innerWidth)
              if (
                ...
                if (scrollY >= startPoint && scrollY <= endPoint) {
                  ...
                  stickyBlockItem.classList.add("down");
                  ...
                  stickyItemValues.width = `${stickyBlockItem.offsetWidth}px`;
                } else stickyBlockItem.classList.remove("down");
      ...
      if (galleries.length) {
        ...
        galleries.forEach((gallery) => {
          galleyItems.push({
            gallery,
            galleryClass: new LightGallery(gallery, lgId++, {
              plugins: [lg_zoom_min],
      ...
      document.addEventListener("DOMContentLoaded", () => {
        ...
        if (!allLanguages.includes(window.location.hash.substring(1)))
          if (localStorage.getItem("defaultLanguage")) {
            location.href = `${window.location.pathname}#${localStorage.getItem(
              "defaultLanguage"
            )}`;
          } else {
            var lang_def = getCookie("langdef");
            if (!lang_def) {
              location.href = `${window.location.pathname}#en`;
            } else {
              location.href = `${window.location.pathname}#${lang_def}`;
            }
          }
        options.forEach((elem) => {
          elem.addEventListener("click", () => {
            location.href = `${window.location.pathname}#${elem.dataset.value}`;
            localStorage.setItem("defaultLanguage", elem.dataset.value);
            changeLanguage();
        ...
        function changeLanguage() {
          ...
          document.querySelector("title").innerHTML =
            languages["pageTitle"][language];
          if (document.querySelectorAll("[data-language]").length > 0)
            document.querySelectorAll("[data-language]").forEach((elem) => {
              elem.classList.remove("_ru");
              elem.classList.remove("_en");
              if (languages[elem.dataset.language])
                if (languages[elem.dataset.language][language])
                  if ("IMG" == elem.tagName) {
                    elem.src = languages[elem.dataset.language][language];
                    elem.classList.add(`_${language}`);
                  } else {
                    elem.innerHTML = languages[elem.dataset.language][language];
                    elem.classList.add(`_${language}`);
                  }
            });
        }
        ...
        if (document.querySelector(".assets")) {
          const listButton = document.querySelector(".assets__button");
          const list = document.querySelector(".assets__books");
          const booksItems = document.querySelectorAll(".books__item");
          const links = document.querySelectorAll(".assets__content a");
          const articles = document.querySelectorAll("[data-book-id]");
          links.forEach((link) => {
            link.addEventListener("click", () => {
              if (!list.classList.contains("assets__books--active")) return;
              else {
                event.preventDefault();
                let id = link.dataset.id;
                console.log(id);
                articles.forEach((article) => {
                  if (article.dataset.bookId == id) {
                    window.scrollTo({
                      top: article.offsetTop - 150,
                      left: article.offsetLeft,
                      behavior: "smooth",
                    });
          ...
          booksItems.forEach((item, i) => {
            let booksScroll = {};
            const innerItems = item.querySelectorAll(".books__inner-item");
            if (innerItems.length)
              innerItems.forEach((item, p) => {
                let tooltips = item.querySelectorAll(".books__tooltip-image");
                if (tooltips.length)
                  tooltips.forEach((tooltip, j) => {
                    tooltip.addEventListener("click", () => {
                      booksScroll.tab = p;
                      booksScroll.section = j;
                      localStorage.setItem(
                        "scrollTo",
                        JSON.stringify(booksScroll)
                      );
                    });
                  });
                item.addEventListener("click", () => {
                  booksScroll.tab = p;
                  localStorage.setItem("scrollTo", JSON.stringify(booksScroll));
                });
              });
          });
          listButton.addEventListener("click", () => {
            listButton.classList.add("assets__button--disappearing");
            list.classList.add("assets__books--active");
        ...
        if (document.querySelector(".header__lower-line")) {
          const headerLine = document.querySelector(".header__lower-line");
          let lastY = 0;
          window.addEventListener("scroll", () => {
            let scrollDistance = window.scrollY;
            if (scrollDistance < lastY) headerLine.classList.add("shown");
            else headerLine.classList.remove("shown");
            lastY = scrollDistance;
          });
        }
      });

      // Переключения языка ютуба
      const getHref = () => {
        ...
        var lang_def = getCookie("langdef");
        var lang_set = getCookie("langset");
        if (!lang_set) {
          var lang_set = localStorage.getItem("defaultLanguage");
        }
        if (!lang_set) {
          var lang_set = lang_def;
        }
        if (lang_set) {
          switch (lang_set) {
            case "ru":
              element.href =
                "https://www.youtube.com/playlist?list=PL3dNiCipcYFMKLzowYfS5ryl1LtDjue11"; // Default Playlist
              break;

            default:
              element.href =
                "https://www.youtube.com/playlist?list=PL3dNiCipcYFN8Xi8JcyxXp6B-WSHxx1B4"; // Eglish Playlist
              break;
          }
        } else {
          element.href =
      ...
      getHref();
      //конец переключения языка ютуба
...
/*
     FILE ARCHIVED ON 12:12:32 Dec 15, 2022 AND RETRIEVED FROM THE
     INTERNET ARCHIVE ON 14:20:00 Oct 23, 2023.
...
/*
playback timings (ms):
  captures_list: 171.149
  exclusion.robots: 0.185
  exclusion.robots.policy: 0.175
  cdx.remote: 0.056
  esindex: 0.009
  LoadShardBlock: 124.527 (3)
  PetaboxLoader3.datanode: 186.105 (4)
  load_resource: 258.0
  PetaboxLoader3.resolve: 116.131
*/
